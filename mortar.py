...
class Bomb:
    class Defect:
        ...
        def get_defect(self):
            ...
            for part in d.keys():
                if "res" in d[part].keys():
                    ...
                    merged_res = merge_lists(
                        d[part]["top"]["res"], d[part]["bot"]["res"]
                    )
            ...
            for cam in res.keys():
                ...
                res[cam] = [
                    ",".join(list(set(sublist))) if sublist else ""
                    for sublist in res[cam]
                ]
        ...
        def get_defect_by_parts(self):
            d = self.defect
            data = {"head": [], "body": [], "powder": [], "wing": [], "anchor": []}

            for part in d.keys():
                if "res" in d[part].keys():
                    data[part] = d[part]["res"]
                else:  # Separated as top and bot
                    data[part] = merge_lists(
                        d[part]["top"]["res"], d[part]["bot"]["res"]
                    )
            ...
            for key, values in data.items():
                res[key] = ",".join(
                    list(set([item for sublist in values for item in sublist if item]))
                )
    ...
    def __init__(self, data_paths):
        ...
        for key in self.infer_stat.keys():
            for stat_conf in infer_stat_conf[key]:
                self.infer_stat[key][stat_conf[0]] = {
                    "total": stat_conf[1],
                    "done": 0,
                    "res": None,
                }
        ...
        for i, row_contents in enumerate(contents):
            ...
            for j, item in enumerate(row_contents):
                if cat1[j] == "" and item[0] is not None:  # New cat1
                    ...
                    buf += " " + item[0]
                    buf += " " * (
                        wcswidth(" └─ ") + max_cat2_len[j] + 8 - wcswidth(item[0])
                    )
