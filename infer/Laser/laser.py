...
class LaserData:
    def __init__(self, file_path):
        ...
        self.data_type = "xyz"  if all(val.strip() == '0.0'  for val in (self.raw_str_data[1])[:-1]) else "z"
        logmg.i.log("data type : %s",self.data_type)
    ...
    def preprocess(self, save_path, dist_to_axis):
        ...
        if self.data_type == "z":
            self.orig_data = {
                "z": np.linspace(-0.01, 0.01, len(data[0])),
                "r": 0.001 * data,
            }
        else:
            self.orig_data = {
    ...
    def __get_defect_removed_rows(self):
        ...
        for idx, line in enumerate(self.raw_str_data):
            try:
                ...
                for i in range(3):
                    idx_buf = idx - idx % 3 + i
                    if idx_buf not in defected_idx[-3:] and idx_buf < len(
                        self.raw_str_data
                    ):
    ...
    def __get_ztr_data(orig_data, dist_to_axis):
        ...
        res = {
            "z": np.linspace(
                orig_data["z"][0], orig_data["z"][-1], orig_data["r"].shape[1]
            ),
...
def get_diff_map(
    ...
    func: Callable = np.min,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    var_reduced_radius = np.zeros(data["r"].shape)
    n_theta, n_z = data["r"].shape

    th_idx_rng = (sz_wnd[0] / 2) // (
        data["r_avg"] * (data["theta"][1] - data["theta"][0])
    )
...
def plot_diff_3d(plot_conf: dict, ca_data: dict, color_base: np.ndarray):
    ...
    fig = go.Figure(
        go.Surface(
            x=ca_data["x"], y=ca_data["y"], z=ca_data["z"], surfacecolor=diff_color
        )
    )
    fig.update_layout(
        scene={
            "xaxis": {"nticks": 7, "range": [-5, 5]},
            "yaxis": {"nticks": 7, "range": [-5, 5]},
            "zaxis": {"nticks": 7, "range": [-5, 5]},
            "aspectmode": "cube",
        }
    )
...
def plot_scatter_3d(out_path, data, diff_map):
    scatter_data = cylindrical_to_map(
        {"r": diff_map, "theta": data["theta"], "z": data["z"]}
    )
...
def run(
    ...
    # Detect defects
    is_crack = is_defected(
        crack_diff_map[0], threshold=-threshold["crack"], over_th=False
    )
...
def infer17(bomb):
    ...
    탄체 균열 또는 뒤틀림, 중부식
    """
    logmg.i.log("# 변위센서 ---- 탄체 중부식")
    conf = laser_conf.infer_15_17

    data_file = bomb.sensor_data_path
    logmg.i.log("데이터 경로 : %s", data_file)

    plot_config = {
        "html": rep_sl_join(
            DIR_PATH["done"]["sensor"]["html"],
            "{}_{}.html".format(bomb.lot.name, bomb.num),
        ),
        "image": rep_sl_join(
            DIR_PATH["done"]["sensor"]["image"],
    ...
    # try:
    is_crack, is_corr, is_shaft_shifted = run(
        data_file,
