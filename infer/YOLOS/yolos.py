...
def infer_6_8(bomb, res_CAM4):
    ...
    if len(c2) > 6:
        ...
        if len(c2) < 12:
            add_defects(DEFECT_CODE['head']['light_corr'][0], c2, bomb.defect['head']['res'])
            is_ok1 = False

        else:
            add_defects(DEFECT_CODE['head']['heavy_corr'][1], c2, bomb.defect['head']['res'])
            is_ok2 = False
...
def infer_15(bomb, res_CAM4):
    ...
    if len(res) > 0:
        ...
        add_defects(DEFECT_CODE['body']['bot']['crack_shift'], res, bomb.defect['body']['top']['res'])
        is_ok = False
...
def infer_16(bomb, res_CAM3):
    ...
    if len(d3) == 0:
        ...
        add_defects(DEFECT_CODE['body']['bot']['ring_exist'], d3, bomb.defect['body']['bot']['res'])
        is_ok = False
        pass
    if is_ok:
        logmg.i.log("정상")
...
def infer_17_24(bomb, res_CAM3, res_CAM4):
    logmg.i.log("# 탄체 부식")
    is_ok1 = True
    is_ok2 = True
    cam3_d4 =get_cls_pos( res_CAM3, "d4")
    cam4_d4 =get_cls_pos( res_CAM4, "d4")

    if len(cam3_d4) + len(cam4_d4) >= 18:
        #이미지 픽셀 개수 이상 확인
        #탄체 중부식추가 - 결함리스트의 위치에 맞는 곳에 추가
        if len(cam3_d4) + len(cam4_d4) <= 36:
            add_defects(DEFECT_CODE['body']['bot']['light_corr'], cam3_d4, bomb.defect['body']['bot']['res'])
            add_defects(DEFECT_CODE['body']['bot']['light_corr'], cam4_d4, bomb.defect['body']['top']['res'])
            is_ok1 = False
            logmg.i.log("소부식")
        else:
            # add_defects(DEFECT_CODE['body']['bot']['heavy_corr'], cam3_d4, bomb.defect['body']['bot']['res'])
            add_defects(DEFECT_CODE['body']['bot']['heavy_corr'], cam4_d4, bomb.defect['body']['top']['res'])
            is_ok2 = False
            logmg.i.log("중부식")
...
def infer_20(bomb, res_CAM4):
    ...
    if len(d5) > 0:
        ...
        add_defects(DEFECT_CODE['body']['top']['filling_leak'], d5, bomb.defect['body']['top']['res'])
        is_ok = False
        pass
    if is_ok:
        logmg.i.log("정상")
...
def infer_21(bomb, res_CAM4):
    ...
    if len(d7) > 0:
        ...
        add_defects(DEFECT_CODE['body']['bot']['ring_damage'], d7, bomb.defect['body']['bot']['res'])
        is_ok = False
        pass
    if is_ok:
        logmg.i.log("정상")
    ...
    def is_defect(cls, position):
        if len(cls) <= 2:
            for i in range(6):
                cnt = get_counter(cls).get(i, 0)
                if cnt == 0:
                    add_defects(DEFECT_CODE['anchor']['exist'], cls, bomb.defect['anchor'][position]['res'])
            return True
        return False
...
def infer_37(bomb, res_CAM1, res_CAM2):
    ...
    def is_defect(target, cam, position=None):
        if len(target) > 0:
            for pos in target:
                row, col = pos
                ...
                quantized_mask =  cv2.convertScaleAbs((mask // 85) * 85)
                # get_mask_img(f"infer/YOLOS/37/{current_time}.png", quantized_mask)
                cv2.imshow("img", quantized_mask)
                cv2.waitKey(0)
                max_sum = np.sum(quantized_mask >= np.max(quantized_mask))
                # maxval = np.max(mask[mask > 0])
                # avgval = np.average(mask[mask > 0])
                unique_values, counts = np.unique(quantized_mask[quantized_mask>0], return_counts=True)
    ...
    logmg.i.log("wc:")
    res_wc = is_defect(wc, res_CAM1, 'bot')
    logmg.i.log("bc:")
    res_bc = is_defect(bc, res_CAM2, 'top')
...
class YOLOInfer:
    ...
    def head_infer(self, bomb):
        ...
        infer_17_24(bomb, res_CAM3, res_CAM4)
