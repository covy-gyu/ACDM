...
def infer_30(bomb):
    ...
    for i, img in enumerate(images):
        ...
        cropped_img = upper_img[crop_rect['top']:crop_rect['bottom'] + 1, crop_rect['left']: crop_rect['right'] + 1]
        value_mask = cv2.inRange(cropped_img, val_threshold['lower'], val_threshold['upper'])
...
def infer_31(bomb):
    ...
    for i, img in enumerate(images):
        ...
        if abs(infer[0][0]) > conf['infer_threshold']:
            result = "부적절한 위치"
            bomb.defect['powder']['top']['res'][i].append(DEFECT_CODE['powder']['top']['position'])
            is_ok = False
...
def infer_32(bomb):
    ...
    for i, img in enumerate(images):
        ...
        if max_idx < 7:
            result = "장력상실 또는 파손"
            bomb.defect['powder']['bot']['res'][i].append(DEFECT_CODE['powder']['bot']['condition'])
            is_ok = False
...
def infer_33(bomb):
    ...
    for i, img in enumerate(images):
        ...
        if score > 5:
            result = "약포 변색"
            bomb.defect['powder']['bot']['res'][i].append(DEFECT_CODE['powder']['bot']['discolor'])
            is_ok = False
...
def infer_34(bomb):
    ...
    for i, img in enumerate(images):
        ...
        if len(means) != 0:
            result = "이물질 감지"
            bomb.defect['powder']['bot']['res'][i].append(DEFECT_CODE['powder']['bot']['fo'])
            is_ok = False
